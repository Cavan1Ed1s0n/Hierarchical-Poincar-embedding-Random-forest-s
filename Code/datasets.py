...
class WordNetDataset(Dataset):
    ...
    def __init__(self,filename='data/wordnet/mammal_closure.tsv',neg_samples = 50):
        ...
        self.items = sorted(tuple(set(A+B))) #individual items from dataset
        
        self.n_items = len(self.items)
        
        self.item2id = {item:i for i,item in enumerate(self.items)} #item2id lookup table

        root_name = self.raw_data[0][0]
        self.root_idx = np.where([item == root_name for item in self.items])[0][0]
        # print(self.root_idx)
